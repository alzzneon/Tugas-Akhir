...
export default function Transmissions() {
  ...
  const endpoint = "/api/admin/masters/transmissions";
  ...
  const formFields = [
    { key: "name", label: "Nama", required: true, placeholder: "Automatic" },
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        await adminFetch(`${endpoint}/${active.id}`, { method: "PUT", body: JSON.stringify(form) });
      }
  ...
  const remove = async (row) => {
    if (!confirm(`Hapus transmisi "${row.name}"?`)) return;
    try {
      setMsg("");
  ...
  return (
    <div>
      ...
      <Modal
        open={open}
        size="md"
        title={mode === "create" ? "Tambah Transmisi" : "Edit Transmisi"}
