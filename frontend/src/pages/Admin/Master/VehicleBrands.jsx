...
export default function VehicleBrands() {
  ...
  const endpoint = "/api/admin/masters/vehicle-brands";
  ...
  const formFields = [
    { key: "name", label: "Nama", required: true, placeholder: "Toyota" },
  ...
  const remove = async (row) => {
    if (!confirm(`Hapus brand "${row.name}"?`)) return;
    try {
      setMsg("");
  ...
  return (
    <div>
      ...
      <Modal
        open={open}
        size="md"
        title={mode === "create" ? "Tambah Merek Kendaraan" : "Edit Merek Kendaraan"}
